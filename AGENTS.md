...
一个基于 FastAPI + Vue 3 的 AI 客服 RAG 系统。用户上传知识库文档后，系统通过向量检索 + 大模型生成回答，并支持用户登录、会话管理、消息上下文、订单查询等能力。
...
│   ├── llm/                # OpenAI 客户端封装
│   ├── rag/                # RAG 全流程：解析、切块、embedding、向量库、检索、prompt
│   ├── repositories/       # 数据访问层：MySQL、Redis、向量库
│   ├── schemas/            # Pydantic 请求/响应模型
│   ├── services/           # 业务编排层
│   ├── tools/              # Tool Calling 工具（如订单查询）
│   ├── uploads/            # 上传文件存储目录
│   └── utils/              # 通用工具函数
├── web/                    # 前端 Vue 3 应用
│   └── src/
...
1. 在 `app/tools/` 下创建新文件，实现 `ChatTool` 协议（`name`、`can_handle`、`call` 三个属性/方法）。
2. 在 `app/dependencies.py` 的 `get_tool_registry()` 中注册新工具。
3. 工具返回的 `ToolExecution.content` 只包含当前用户可见的数据。
4. 所有工具查询必须通过 `user_id` 做数据隔离。
...
- 使用 PyMySQL，通过 `MySQLRepository` 的 context manager 管理连接。
- 所有写操作使用事务（已在 `connection()` 中自动 commit/rollback）。
- 所有查询必须使用参数化 SQL（`%s` 占位符），禁止字符串拼接。
- 查询结果通过 `_map_xxx()` 函数转换为 frozen dataclass。
...
```
文档上传 → 文本解析(parser) → 切块(splitter) → Embedding(embedding) → 存入向量库(milvus_db)
```

用户提问时的处理链路：

```
用户问题 → Embedding → 向量检索(milvus_db) → 获取相关 chunks
         → Tool Calling(如有匹配) → 拼接 Prompt(prompt) → 调用 LLM → 返回回答
```
...
- 禁止直接修改 `AI_RULES.md`、`PROJECT_PLAN.md`、`README.md`、`schema.sql`、`.env`、`.env.example`，除非用户明确要求。
