...
from app.dependencies import get_current_manager, get_session_service
from app.repositories.mysql.records import ChatSessionRecord, ManagerRecord
...
def list_sessions(
    ...
    """
    sessions = service.list_sessions(manager.id, pageNum, pageSize)
...
def create_session(
    ...
    Args:
        ...
        service: 当前接口注入的业务服务实例。
    """
    session = service.create_session(manager.id, request.sessionTitle, request.sessionContent, request.remark)
...
def update_session(
    ...
    Args:
        ...
        service: 当前接口注入的业务服务实例。
    """
    session = service.update_session(manager.id, request.id, request.sessionTitle, request.sessionContent, request.remark)
    if not session:
...
def get_session(
    ...
    Args:
        ...
        service: 当前接口注入的业务服务实例。
    """
    session = service.get_session(manager.id, session_id)
    if not session:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="会话不存在")
...
def delete_session(
    ...
    Args:
        ...
        service: 当前接口注入的业务服务实例。
    """
    service.delete_sessions(manager.id, [item for item in ids.split(",") if item])
